...
def dyad_integer_values(frag_start, frag_len, dyad_match_mode):
    """
    Return dyad keys as integers for one BAM/chromosome-local matching.
...
def iter_fragment_pairs_from_bam_chrom_lengths(
    ...
    for aln in iterator:
        ...
        if max_duplicates >= 0:
            ...
            # Preserves the behaviour of the earlier script: max_duplicates=N
            # keeps N+1 observations of the same coordinate.
...
def build_chrom_match_sets(
    ...
    """
    Build matched coordinate sets for one BAM chromosome only.

    This is the RAM-saving replacement for the old genome-wide PASS1. It stores
    only the keys needed for the selected matching mode, and clears them after
    the chromosome has been profiled.
    """
    length_a, length_b = lengths
    coords_by_len = {length: set() for length in lengths}

    if match_mode == "dyad":
        dyad_to_coords_by_len = {length: defaultdict(set) for length in lengths}
    elif match_mode == "ends":
        left_to_coords_by_len = {length: defaultdict(set) for length in lengths}
        right_to_coords_by_len = {length: defaultdict(set) for length in lengths}
    else:
    ...
    for _aln1, _aln2, frag_start, frag_end, frag_len in iter_fragment_pairs_from_bam_chrom_lengths(
        ...
        coords_by_len[frag_len].add(coord_id)
        ...
        if match_mode == "dyad":
            for dyad_key in dyad_integer_values(
                ...
                dyad_to_coords_by_len[frag_len][dyad_key].add(coord_id)
        else:
            left_to_coords_by_len[frag_len][frag_start].add(coord_id)
            right_to_coords_by_len[frag_len][frag_end].add(coord_id)
    ...
    if match_mode == "dyad":
        ...
        matched = {length: {DYAD_SUBSET: set()} for length in lengths}

        shared_dyads = set(dyad_to_coords_by_len[length_a]).intersection(
            dyad_to_coords_by_len[length_b]
        )

        for dyad_key in shared_dyads:
            matched[length_a][DYAD_SUBSET].update(dyad_to_coords_by_len[length_a][dyad_key])
            matched[length_b][DYAD_SUBSET].update(dyad_to_coords_by_len[length_b][dyad_key])

    else:
        subset_names = BASE_SUBSETS
        matched = {length: {subset: set() for subset in subset_names} for length in lengths}

        shared_lefts = set(left_to_coords_by_len[length_a]).intersection(
            left_to_coords_by_len[length_b]
        )
        shared_rights = set(right_to_coords_by_len[length_a]).intersection(
            right_to_coords_by_len[length_b]
        )

        for left in shared_lefts:
            matched[length_a]["left"].update(left_to_coords_by_len[length_a][left])
            matched[length_b]["left"].update(left_to_coords_by_len[length_b][left])

        for right in shared_rights:
            matched[length_a]["right"].update(right_to_coords_by_len[length_a][right])
            matched[length_b]["right"].update(right_to_coords_by_len[length_b][right])

        for length in lengths:
            matched[length]["any"] = matched[length]["left"] | matched[length]["right"]

    return coords_by_len, matched


def profile_chrom_matched_fragments(
    bam,
    bam_chrom,
    lengths,
    subset_names,
    ...
    """
    Rescan one BAM chromosome and profile fragments whose coordinate was matched
    in build_chrom_match_sets().
    """
    coord_counts = defaultdict(int)
    ...
    for aln1, aln2, frag_start, frag_end, frag_len in iter_fragment_pairs_from_bam_chrom_lengths(
        ...
        matched_subsets = [
            ...
            if coord_id in matched[frag_len][subset]
        ]
        ...
        if progress["profiled_matching_fragments"] % 100000 == 0:
            print(
                f"[PROFILE] reconstructed/fetched matching fragments: {progress['profiled_matching_fragments']:,}",
                file=sys.stderr,
            )
...
def process_bams_lowmem(
    ...
    Low-memory processing:
      for each BAM
        for each chromosome
          build matched coordinate sets for this chromosome only
          rescan and add matched fragments to aggregate profiles
          clear chromosome state
    """
    subset_names = (DYAD_SUBSET,) if match_mode == "dyad" else BASE_SUBSETS
    profiles = make_profiles(lengths, subset_names, profile_specs)

    total_pairs_by_len = {length: 0 for length in lengths}
    unique_coords_count_by_len = {length: 0 for length in lengths}
        ...
        for bam_index, bam_path in enumerate(bam_files):
            ...
            with pysam.AlignmentFile(bam_path, "rb") as bam:
                ...
                for bam_chrom in bam_chroms:
                    ...
                    print(
                        f"[MATCH] Scanning {os.path.basename(bam_path)} {bam_chrom}",
                        file=sys.stderr,
                    )

                    coords_by_len, matched = build_chrom_match_sets(
                        bam=bam,
                        bam_chrom=bam_chrom,
                        lengths=lengths,
                    ...
                    for length in lengths:
                        unique_coords_count_by_len[length] += len(coords_by_len[length])
                        for subset in subset_names:
                            matched_coords_count[length][subset] += len(matched[length][subset])

                    match_parts = []
                    for length in lengths:
                        for subset in subset_names:
                            match_parts.append(
                                f"len{length}_{subset}={len(matched[length][subset]):,}"
                            )
                    print(
                        f"[MATCH] {os.path.basename(bam_path)} {bam_chrom}: "
                        + "; ".join(match_parts),
                        file=sys.stderr,
                    )

                    if not any(
                        len(matched[length][subset]) > 0
                        for length in lengths
                        for subset in subset_names
                    ):
                        continue
                    ...
                    reached_limit = profile_chrom_matched_fragments(
                        bam=bam,
                        bam_chrom=bam_chrom,
                        lengths=lengths,
        ...
        for length in lengths:
            for subset in subset_names:
                ...
                row = [
                    str(length),
                    subset,
                    str(total_pairs_by_len[length]),
                    str(unique_coords_count_by_len[length]),
                    str(matched_coords_count[length][subset]),
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            ...
            "using either fragments with shared left/right ends OR fragments with shared dyads "
            "between the two lengths. This low-memory version matches and profiles one BAM "
            "chromosome at a time instead of storing genome-wide coordinate sets."
        )
    )
    ...
    parser.add_argument(
        ...
        default="ends",
        help=(
            "Which fragments to profile. 'ends' profiles fragments sharing left or right ends "
            "with the other length and writes shared_any/shared_left/shared_right outputs. "
            "'dyad' profiles only fragments sharing the same dyad/centre with the other length "
            "and writes shared_dyad outputs. These modes are mutually exclusive. Default: ends."
        ),
    )
    ...
    parser.add_argument(
        ...
        default="split",
        help=(
            "How to define shared dyads when --match-mode dyad is used. "
            "'split' is the default and mimics integer dyad tracks by assigning odd-length "
            "fragments to their central base and even-length fragments to both middle bases. "
            "This allows odd/even length pairs such as 145 and 162 to share a dyad. "
            "'exact' uses half-base-safe geometric centres, where odd/even length pairs cannot "
            "exactly match. 'floor' uses the left/lower integer centre for even lengths; "
            "'ceil' uses the right/higher integer centre for even lengths."
        ),
    )
    ...
    parser.add_argument(
        ...
        required=True,
        help=(
            "Output prefix. In --match-mode ends, the script writes "
            "<prefix>_len<L>_shared_any.tsv, <prefix>_len<L>_shared_left.tsv, "
            "<prefix>_len<L>_shared_right.tsv, and <prefix>_summary.tsv. "
            "In --match-mode dyad, it writes <prefix>_len<L>_shared_dyad.tsv "
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "This preserves the previous script's behaviour, where N keeps N+1 observations."
        ),
    )
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Optional maximum number of matched fragments to reconstruct/fetch in the profile pass. "
            "Useful for testing only. Unlike the previous version, this can stop processing early."
        ),
    )
    ...
    print("[INFO] low-memory mode: processing one BAM chromosome at a time", file=sys.stderr)
    ...
    for length in lengths:
        print(
            f"[INFO] length {length}: {total_pairs_by_len[length]:,} fragments seen after filters; "
            f"{unique_coords_count_by_len[length]:,} unique coordinates after coordinate deduplication",
            file=sys.stderr,
        )

        parts = []
        for subset in subset_names:
            parts.append(f"{subset}-shared={matched_coords_count[length][subset]:,}")
        print(f"[INFO] length {length}: " + "; ".join(parts), file=sys.stderr)

    if all(
        matched_coords_count[length][subset] == 0
        for length in lengths
        for subset in subset_names
    ):
    ...
    write_summary_tsv(
        ...
        subset_names=subset_names,
        total_pairs_by_len=total_pairs_by_len,
        unique_coords_count_by_len=unique_coords_count_by_len,
    ...
    print(
        "[INFO] This low-memory version does not keep genome-wide coordinate sets. "
        "It builds matches for one BAM chromosome, profiles those matches, then clears that chromosome.",
        file=sys.stderr,
    )
    ...
    if args.match_mode == "dyad":
        print(
            ...
            "both integer middle positions.",
            file=sys.stderr,
        )
        print(
            "[INFO] In --match-mode dyad, left/right ends are not used and no shared_any/shared_left/shared_right "
            "outputs are written.",
            file=sys.stderr,
        )
    else:
        print(
            "[INFO] shared_any is the union of shared_left and shared_right. In --match-mode ends, "
            "dyads are not used and no shared_dyad output is written.",
            file=sys.stderr,
        )
