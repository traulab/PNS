...
ROMAN_TO_INT_1_TO_16 = {roman: i + 1 for i, roman in enumerate(ROMAN_NUMERALS_1_TO_16)}

# NCBI RefSeq accessions used by the S. cerevisiae S288C/R64 assembly.
# These are included as aliases so commands like --chroms I or --chroms 1-16
# can still work when the BAM/FASTA contig names are NC_001133.9, etc.
YEAST_ROMAN_TO_NCBI = {
    "I": "NC_001133",
    "II": "NC_001134",
    "III": "NC_001135",
...
YEAST_MT_ALIASES = {
    "M", "MT", "MITO", "MITOCHONDRION", "MITOCHONDRIAL",
    "CHRM", "CHRMT", "MTRDNA", "MTDNA", "NC_001224",
}


def _strip_chr_prefix(token):
...
def _roman_for_int_string(token):
    if str(token).isdigit():
        value = int(token)
        if 1 <= value <= 16:
            return ROMAN_NUMERALS_1_TO_16[value - 1]
    return None


def expand_chroms(chrom_spec):
    """
    Return requested chromosome tokens.

    Important yeast-compatible change:
      --chroms all / auto / bam returns None, meaning "use all contigs found
      in each BAM". The previous human-specific behaviour was hard-coded to
      1..22,X,Y, which causes yeast BAMs to be skipped.

    Useful yeast options:
      --chroms all       all BAM contigs
      --chroms yeast     chromosomes I..XVI
      --chroms I,II,III  roman chromosome names
      --chroms 1-16      numeric shorthand, later matched to chrI/NC_001133.*
    """
    chrom_spec = chrom_spec.replace(" ", "")

    if chrom_spec.lower() in {"all", "auto", "bam"}:
        return None

    if chrom_spec.lower() in {"yeast", "saccer", "saccer3", "r64", "s288c"}:
        return ROMAN_NUMERALS_1_TO_16.copy()
    ...
    for token in chrom_spec.split(","):
        ...
        token_no_chr = _strip_chr_prefix(token)

        if "-" in token_no_chr:
            start, end = token_no_chr.split("-", 1)

            start_roman = _roman_for_int_string(start)
            end_roman = _roman_for_int_string(end)

            if start.isdigit() and end.isdigit():
                chroms.extend([str(i) for i in range(int(start), int(end) + 1)])
            elif start_roman and end_roman:
                start_i = ROMAN_TO_INT_1_TO_16[start_roman]
                end_i = ROMAN_TO_INT_1_TO_16[end_roman]
                chroms.extend(ROMAN_NUMERALS_1_TO_16[start_i - 1 : end_i])
            elif start.upper() in ROMAN_TO_INT_1_TO_16 and end.upper() in ROMAN_TO_INT_1_TO_16:
                start_i = ROMAN_TO_INT_1_TO_16[start.upper()]
                end_i = ROMAN_TO_INT_1_TO_16[end.upper()]
                chroms.extend(ROMAN_NUMERALS_1_TO_16[start_i - 1 : end_i])
            else:
                raise ValueError(f"Invalid chromosome range: {token}")
        else:
            chroms.append(token_no_chr)
...
def contig_aliases(chrom):
    """
    Generate common aliases for human-style and yeast-style contig names.

    This handles:
      1 <-> chr1
      I <-> chrI <-> 1 <-> chr1 <-> NC_001133.*
      MT/M/chrM/chrMT <-> NC_001224.*
    """
    original = str(chrom).strip()
    no_chr = _strip_chr_prefix(original)
    upper_no_chr = no_chr.upper()

    aliases = []
    for token in (original, no_chr):
        if token and token not in aliases:
            aliases.append(token)

    # Generic chr/no-chr aliases.
    if no_chr:
        aliases.append("chr" + no_chr)
        aliases.append("Chr" + no_chr)

    # Yeast mitochondrial aliases.
    if upper_no_chr in YEAST_MT_ALIASES:
        aliases.extend([
            "M", "MT", "chrM", "chrMT", "Mito", "Mitochondrion",
            "mitochondrion", "NC_001224",
        ])

    # Yeast nuclear chromosome aliases from numeric or roman input.
    roman = None
    if upper_no_chr in ROMAN_TO_INT_1_TO_16:
        roman = upper_no_chr
    elif no_chr.isdigit() and 1 <= int(no_chr) <= 16:
        roman = ROMAN_NUMERALS_1_TO_16[int(no_chr) - 1]

    if roman is not None:
        num = ROMAN_TO_INT_1_TO_16[roman]
        ncbi_prefix = YEAST_ROMAN_TO_NCBI[roman]
...
def matching_contig(chrom, references):
    ...
    aliases = contig_aliases(chrom)

    for alias in aliases:
        if alias in references:
            return alias

    # NCBI accessions may include version suffixes in FASTA/BAM names.
    # Allow NC_001133 to match NC_001133.9, etc.
    for alias in aliases:
...
def main():
    ...
    parser.add_argument(
        ...
        default="all",
        help=(
            "Chromosomes/contigs to analyse. Yeast-compatible: all/auto/bam uses all BAM contigs; "
            "yeast/r64/s288c uses chrI..chrXVI aliases; also accepts I,II,III or 1-16. "
            "Default: all."
        ),
    )
    ...
    for bam_path in bam_files:
        ...
        if requested_chroms is None:
            # Yeast-compatible all-contigs mode. This avoids the old human-specific
            # 1..22,X,Y assumption and works with NCBI accessions such as NC_001133.9,
            # UCSC-style chrI names, SGD-style roman names, or any custom contig names
            # present in the BAM.
            bam_chroms = list(bam.references)
            missing_chroms = []
            print(
                f"[INFO] --chroms {args.chroms}: using all {len(bam_chroms)} BAM contigs.",
                file=sys.stderr,
            )
        else:
            bam_chroms = []
            ...
            bam_chroms = list(dict.fromkeys(bam_chroms))
